...
thromboSeqANOVA <- function(dge = dgeIncludedSamples,
  ...
  #   iteration: for digitalSWARM store temporary output
  ...
  if (!is.null(iteration)){
    save(normalized.counts, file = paste(iteration,"-normalized.counts.RData",sep=""))
    dgeRUV <- dge
    save(dgeRUV, file = paste(iteration,"-RUVcorrectionSettings.RData",sep=""))  
  }
  ...
  if (plot == TRUE){
    ...
    heatmap.fisher <- fisher.test(table(data.frame(cbind(as.matrix(slice(
      ...
      as.character(dge$samples[
        rownames(as.matrix(slice(h$colDendrogram, k = nlevels(dge$samples$group)))),
        "group"])))))
    print(paste("PSO-selected fdr threshold: ", round(as.numeric(result$par), digits = 6)), sep = "")
